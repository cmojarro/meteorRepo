...
Template.employeeRow.events({
	'dblclick .employeeRow':function(evt,templates){
		Session.set('editing_employee',templates.data._id);
...
Template.projectForm.events({
	'click .save':function(evt,templates){
		var name = templates.find('.name').value;
		var client = templates.find('.client').value;
		var status = templates.find('.status').value;
		var owner = templates.find('.owner').value;
		var datedue = templates.find('.datedue').value;
	...
	'click .cancel':function(evt,templates){
		Session.set('showProjectDialog',false);
		Session.set('editing_project',null);
	},
	'click .remove':function(evt,templates){
		removeProject();	
		Session.set('showProjectDialog',false);
		Session.set('editing_project',null);
	}
...
Template.projects.events({
	'click .addProject':function(evt,templates){
...
Template.projectRow.events({
	'dblclick .projectRow':function(evt,templates){
		Session.set('editing_project',templates.data._id);
		Session.set('showProjectDialog',true);
	}
})
...
Template.employees.events({
	'click .addEmployee':function(evt,templates){
		Session.set('showAddEmployee',true);
	}
})
